...
- 本文档是 Menu 组件的初始计划。
- **当前状态：⚠️ 部分实现**
- `src/ui/widgets/menu.ts` 已实现基本 Menu 组件
- `src/ui/widgets/menu_item.ts` 已实现 MenuItem
- `src/ui/base/top_layer.ts` 已提供 light dismiss 支持
...
- 与现有 Dropdown/DropdownMenu 的能力对齐或替代（评估后决定是否迁移）⚠️
...
- 抽出公共 Menu 组件，让 DropdownMenu 内部调用 Menu（推荐）⚠️
- 新建 Menu 组件，先不改 DropdownMenu，后续逐步迁移（低风险）
