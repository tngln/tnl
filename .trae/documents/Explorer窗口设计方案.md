...
M1：Explorer 窗口骨架
- 新增窗口 + Surface
- 顶部地址栏（breadcrumb + 输入框）+ Refresh
- List 模式列出“虚拟目录 + 文件”，支持进入/返回/上级
- 右侧 Details 面板显示选中条目元信息（不含缩略图）

M2：Thumbnails 模式
- Tile 网格布局
- 视频缩略图生成（惰性队列）+ 缓存到 `.tnl-cache/thumbs`
- Details 面板支持缩略图/视频信息展示

M3：操作完善
- Import/Export/Delete/Rename/Move
- Show hidden / Filter（如需要）
