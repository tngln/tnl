## 现状更新
- 本文档是 TopLayer 浮层系统的初始计划。
- **当前状态：✅ 已实现**
- `src/ui/base/top_layer.ts` 已实现：
  - `TopLayerController` 类
  - `TopLayerHost` UIElement
  - `open()` / `close()` / `closeAll()` / `isOpen()` / `containsPoint()` / `lightDismiss()` API
- `src/ui/widgets/dropdown.ts` 已集成 TopLayer
