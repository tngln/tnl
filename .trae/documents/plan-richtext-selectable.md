## 现状更新
- 本文档是 RichText 可选中功能的初始计划。
- **当前状态：⚠️ 部分实现**
- `src/platform/web/1px_textarea.ts` 已实现多行 textarea 桥接
- `src/platform/web/1px_text_control.ts` 已实现公共逻辑
- `src/ui/widgets/rich_text_selectable.ts` 已实现可选中 RichText widget
- `src/ui/builder/components.tsx` 已支持 `selectable` prop
- `src/ui/window/about_dialog.tsx` 已启用 selectable
