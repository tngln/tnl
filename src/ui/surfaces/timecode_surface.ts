...
export class TimecodeSurface implements Surface {
  ...
  render(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, viewport: ViewportContext) {
    ...
    draw(
      ...
      RRect({ x: 10, y: 10, w: Math.max(0, viewport.contentRect.w - 20), h: Math.max(0, viewport.contentRect.h - 20), r: 12 }, { fill: { color: theme.colors.white03 }, stroke: { color: theme.colors.white10, hairline: true }, pixelSnap: true }),
