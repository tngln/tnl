...
3. Declarative surfaces turn `RenderElement` trees into stable retained nodes and pure paint/layout output.
...
- Render handlers must declare their runtime class explicitly as `primitive`, `control`, or `widget`; nodes like selectable rich text may resolve that class from props instead of relying on convention.

Declarative UI code should choose among those node classes intentionally instead of creating parallel lifecycle systems.
...
- render declaration snapshots and retained-runtime snapshots side by side for declarative surfaces
...
- Declarative render call sites still distinguish `mountWidget()` and `mountControl()` even though the runtime path is now unified.
